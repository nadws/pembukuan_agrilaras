<x-theme.app title="{{ $title }}" table="Y" sizeCard="11">
    ...
    <x-slot name="cardBody">
        ...
        <form action="{{ route('penjualan2.save_bayar_piutang') }}" method="post" class="save_jurnal">
            @csrf
                ...
                <div class="col-lg-12">
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            $hutang = DB::selectOne("SELECT a.tgl, a.kode, a.urutan, GROUP_CONCAT(b.nm_produk) AS
                            nm_produk_concat, sum(a.qty) as qty, sum(a.total_rp) as ttl_rp, c.total_bayar,
                            d.nm_customer, a.id_customer, a.lokasi
                            ...
                            <tr>
                                ...
                                <td>{{$hutang->kode}}-{{$hutang->urutan}}</td>
                                <td>{{$hutang->nm_produk_concat}}</td>
                                <td>
                                    @if ($hutang->lokasi == 'mtd')
                                    {{$hutang->id_customer}}
                                    @else
                                    {{$hutang->nm_customer}}
                                    @endif
                                ...
                                <td align="right">{{number_format($hutang->qty,0)}}</td>
                                <td align="right">Rp {{number_format($hutang->ttl_rp,0)}}</td>
                                <td align="right">Rp {{number_format($hutang->total_bayar,0)}}</td>
                                <td>
                                    <input type="text" class="form-control bayar bayar{{$no+1}}" count="{{$no+1}}"
                                        style="text-align: right"
                                        value="Rp {{number_format($hutang->total_bayar,0,',','.')}}">
                                    <input type="hidden" name="pembayaran[]"
                                        class="form-control bayar_biasa bayar_biasa{{$no+1}}" style="text-align: right"
                                        value="{{$hutang->total_bayar}}">
                                    <input type="hidden" class="batas{{$no+1}}" value="{{$hutang->total_bayar}}">
                            ...
                            @php
                            $nm_customers[] = $hutang->lokasi == 'mtd'
                            ? $hutang->id_customer : $hutang->nm_customer;
                            @endphp
                            @endforeach
                            @php
