<x-theme.app title="Piutang" table="Y" sizeCard="12">
    ...
    <x-slot name="cardBody">
        ...
        <form method="GET" action="{{ route('transaksi.piutang.pelunasan') }}" id="formPilihPiutang">
            ...
            <div class="receivable-table-wrap"><table class="table table-hover align-middle receivable-table"><thead><tr><th class="text-center">Pilih</th><th>No</th><th>Tanggal</th><th>No Nota</th><th>Customer</th>@if($jenis === 'telur')<th>Tipe</th>@elseif($jenis === 'ayam')<th class="text-end">Qty Ekor</th>@else<th class="text-end">Qty Item</th>@endif<th class="text-end">Total Piutang</th><th class="text-end">Sudah Dibayar</th><th class="text-end">Sisa Piutang</th><th>Status</th></tr></thead><tbody>@forelse($piutang as $i => $item)<tr><td class="text-center"><input type="checkbox" class="form-check-input nota-piutang" name="nota[]" value="{{ $item->no_nota }}" data-customer="{{ $item->id_customer }}"></td><td>{{ $i + 1 }}</td><td>{{ tanggal($item->tgl) }}</td><td class="fw-semibold">{{ $item->no_nota }}</td><td>{{ $item->nm_customer ?? '-' }}</td>@if($jenis === 'telur')<td>{{ strtoupper($item->tipe) }}</td>@else<td class="text-end">{{ number_format($item->qty,0,'.',',') }}</td>@endif<td class="text-end receivable-total">Rp {{ number_format($item->nilai_piutang,0,'.',',') }}</td><td class="text-end receivable-paid">Rp {{ number_format($item->jumlah_dibayar,0,'.',',') }}</td><td class="text-end receivable-remaining">Rp {{ number_format($item->sisa_piutang,0,'.',',') }}</td><td>@if($item->jumlah_dibayar > 0)<span class="badge bg-warning text-dark">Dicicil</span>@else<span class="badge bg-danger">Belum Dibayar</span>@endif</td></tr>@empty<tr><td colspan="10" class="receivable-empty">Tidak ada piutang {{ $jenis }} pada periode ini.</td></tr>@endforelse</tbody></table></div>
        </form>
        <script>
