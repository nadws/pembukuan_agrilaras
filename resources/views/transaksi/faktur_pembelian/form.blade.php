<x-theme.app title="{{ $title }}" table="Y" sizeCard="12">
    <x-slot name="cardHeader">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div>
                ...
                <small class="text-muted">{{ isset($faktur) ? $faktur->no_faktur : 'Input transaksi pembelian pakan, vitamin/obat/vaksin, atau barang umum dari pemasok' }}</small>
            </div>
            <a href="{{ (isset($faktur) ? route('transaksi.faktur-pembelian.detail', $faktur) : route('transaksi.faktur-pembelian.index')) }}" class="btn btn-outline-primary btn-sm">
    ...
    <x-slot name="cardBody">
        ...
        <form method="POST" action="{{ (isset($faktur) ? route('transaksi.faktur-pembelian.update', $faktur) : route('transaksi.faktur-pembelian.store')) }}" class="invoice-form"
            id="form-faktur">
            @csrf
            @isset($faktur)
                @method('PUT')
            @endisset
            @php
                $jenisFakturTerpilih = old('jenis_faktur', isset($faktur) ? ($faktur->jenis_faktur === 'vaksin' ? 'vitamin' : $faktur->jenis_faktur) : null);
                $biayaLama = collect($faktur->biaya_lain ?? []);
            @endphp
            ...
            <div class="invoice-entry {{ $jenisFakturTerpilih ? '' : 'is-hidden' }}" id="invoice-entry">
                    ...
                    <div class="row g-3">
                        <div class="col-lg-3 col-md-6">
                            ...
                            <input type="text" id="no_faktur" name="no_faktur" class="form-control"
                                value="{{ old('no_faktur', $faktur->no_faktur ?? $noFakturDefault) }}" required readonly>
                        </div>
                        <div class="col-lg-3 col-md-6">
                            ...
                            <input type="date" id="tanggal_faktur" name="tanggal_faktur" class="form-control"
                                value="{{ old('tanggal_faktur', $faktur->tanggal_faktur ?? now()->toDateString()) }}" required>
                        </div>
                        <div class="col-lg-3 col-md-6">
                            ...
                            <select id="supplier_id" name="supplier_id" class="form-select select-search-supplier"
                                ...
                                @foreach ($suppliers as $s)
                                    <option value="{{ $s->id_suplier }}" @selected(old('supplier_id', $faktur->supplier_id ?? null) == $s->id_suplier)>
                                        {{ $s->nm_suplier }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg-3 col-md-6">
                            ...
                            <input type="text" id="keterangan" name="keterangan" class="form-control"
                                value="{{ old('keterangan', $faktur->keterangan ?? null) }}" placeholder="Catatan tambahan (opsional)">
                        </div>
                    </div>
                </div>
                                ...
                                <tbody>
                                    @foreach(['ongkir' => 'Biaya Ekspedisi / Ongkir', 'admin' => 'Biaya Admin'] as $kode => $label)
                                        <tr>
                                            ...
                                            <td>
                                                <input type="number" min="0" step="0.01" name="biaya_lain[{{ $kode }}][nominal]" class="form-control input-biaya-lain" value="{{ old('biaya_lain.'.$kode.'.nominal', data_get($biayaLama->firstWhere('kode', $kode), 'nominal', 0)) }}" placeholder="0">
                                            </td>
                                        </tr>
                                    @endforeach
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" min="0" step="0.01" name="pph23_manual" class="form-control input-pph23-manual" value="{{ old('pph23_manual', $biayaLama->sum('pph23_nominal')) }}" placeholder="0">
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div class="d-flex gap-2 justify-content-end">
                    <a href="{{ (isset($faktur) ? route('transaksi.faktur-pembelian.detail', $faktur) : route('transaksi.faktur-pembelian.index')) }}" class="btn btn-outline-secondary">
                        Batal
                    </a>
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save me-1"></i> {{ isset($faktur) ? 'Simpan Perubahan' : 'Simpan Faktur' }}
                    </button>
                </div>
            </div>
        </form>
    ...
    @section('scripts')
        <script>
            (function() {
                ...
                const initialItems = @json(isset($itemsLama) ? $itemsLama->values() : array_values(old('item', [])));
                let indexBaris = 0;
                ...
                function tambahBaris(data = {}) {
                    ...
                    Array.from(selectProduk.options).forEach((option) => {
                        if (option.value && !produkCocok(option.dataset.kategori)) option.remove();
                    });
                    barisBaru.querySelector('.input-sumber-produk').value = jenisTerpilih() === 'barang_umum' ? 'barang_umum' : 'perencanaan';
                ...
                function filterProduk() {
                    ...
                    tbody.querySelectorAll('.select-produk').forEach((select) => {
                        ...
                        const previousCategory = select.options[select.selectedIndex]?.dataset.kategori;
                        select.innerHTML = template.content.querySelector('.select-produk').innerHTML;
                        Array.from(select.options).forEach((option) => {
                            if (!option.value) return;

                            if (!produkCocok(option.dataset.kategori)) option.remove();
                        });

                        select.value = produkCocok(previousCategory) ? previousValue : '';
