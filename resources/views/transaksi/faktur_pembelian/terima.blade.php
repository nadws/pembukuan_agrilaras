    ...
    <x-slot name="cardHeader">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div>
                ...
                <small class="text-muted">Terima barang dari faktur {{ $faktur->no_faktur }}</small>
            </div>
        ...
        <div class="receive-page">
            @if ($sudahDiterima)
                <div class="alert alert-success">
                    Semua stok untuk faktur ini sudah diterima.
                </div>
            @endif
            ...
            @if ($faktur->jenis_faktur === 'pakan')
                <div class="alert alert-info">
                    Qty diterima diisi dalam zak. Saat masuk stok, 1 zak dihitung sebagai 50.000 gram. Harga HPP sudah termasuk alokasi ongkir dan admin.
                </div>
            @elseif(count($faktur->biaya_lain ?? []))
                <div class="alert alert-info">Harga HPP stok sudah termasuk alokasi ongkir dan admin dari faktur.</div>
            @endif

            <form method="POST" action="{{ route('transaksi.faktur-pembelian.terima.store', $faktur) }}"
                id="form-terima-stok">
                @csrf
                ...
                <div class="row align-items-end g-3 mb-3">
                    <div class="col-lg-3 col-md-6">
                        ...
                        <input type="date" id="tanggal_terima" name="tanggal_terima" class="form-control"
                            value="{{ old('tanggal_terima', now()->toDateString()) }}" @disabled($sudahDiterima)
                            required>
                    </div>
                </div>
                    ...
                    <table class="table receive-table">
                        ...
                        <tbody>
                            @foreach ($faktur->detail as $no => $detail)
                                @php
                                    $qtyDiterima = (float) ($qtyDiterimaByProduk[$detail->pakan_id] ?? 0);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" step="0.01" min="0.01"
                                            ...
                                            data-harga="{{ $hargaHpp }}" @disabled($sudahDiterima)
                                            required>
                                    </td>
                                    <td>{{ $detail->satuan ?? '-' }}</td>
                                    ...
                                    <td class="text-end subtotal-terima">Rp
                                        {{ number_format($qtySisa * $hargaHpp, 0, ',', '.') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="row justify-content-end mt-3">
                    <div class="col-lg-4 col-md-6">
                        <div class="receive-total">
                            ...
                            <div class="value" id="total-terima">Rp
                                {{ number_format($faktur->total_harga, 0, ',', '.') }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-3">
                    ...
                    <button type="submit" class="btn btn-primary" @disabled($sudahDiterima)>
