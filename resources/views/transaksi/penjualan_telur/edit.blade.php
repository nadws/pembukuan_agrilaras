    ...
    <x-slot name="cardBody">
        ...
        <form method="POST" action="{{ route('transaksi.penjualan-telur.update', $nota->no_nota) }}" id="form-edit-penjualan">
            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-md-3"><label class="form-label">Customer 1</label><select name="id_customer" class="form-select select2" required><option value="">Pilih customer 1</option>@foreach ($customers as $customer)<option value="{{ $customer->id_customer }}" @selected(old('id_customer', $nota->id_customer) == $customer->id_customer)>{{ $customer->nm_customer }}</option>@endforeach</select></div>
                <div class="col-md-3"><label class="form-label">Customer 2</label><select name="id_customer2" class="form-select select2"><option value="">Pilih customer 2 (opsional)</option>@foreach ($customers as $customer)<option value="{{ $customer->id_customer }}" @selected(old('id_customer2', $nota->id_customer2) == $customer->id_customer)>{{ $customer->nm_customer }}</option>@endforeach</select></div>
