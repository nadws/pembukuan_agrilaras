@foreach ($rows as $row)
    @php
        $displayValue = bcmul($row['value'], (string) ($multiplier ?? 1), 12);
        $detailStartDate = $reportDate->copy()->startOfMonth()->toDateString();
    @endphp
    <tr class="account-row" data-account-name="{{ \Illuminate\Support\Str::lower($row['kode'] . ' ' . $row['nama']) }}">
        <td>
            <a href="{{ route('pembukuan-baru.buku-besar.detail', ['id' => $row['id'], 'tgl1' => $detailStartDate, 'tgl2' => $reportDate->toDateString()]) }}"
