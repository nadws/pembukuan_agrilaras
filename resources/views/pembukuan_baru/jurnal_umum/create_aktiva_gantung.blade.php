<x-theme.app title="{{ $title }}" table="Y" sizeCard="10">
    <x-slot name="cardHeader">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div>
                ...
                <small class="text-muted">Menampung biaya pembangunan/pengadaan aset sebelum jadi aktiva tetap</small>
            </div>
            <a href="{{ route('pembukuan-baru.jurnal-umum.index', ['kelompok' => 'aktiva-gantung']) }}"
                class="btn btn-outline-primary btn-sm">
                <i class="fas fa-arrow-left me-1"></i> Aktiva Gantung
            </a>
        </div>
    </x-slot>
    ...
    <x-slot name="cardBody">
        ...
        @php
            $modeAset = old('mode_aset', 'baru');
            $oldDetail = old('detail', [
                ['keterangan' => '', 'jumlah' => ''],
            ]);
        @endphp

        <form method="POST" action="{{ route('pembukuan-baru.jurnal-umum.aktiva-gantung.store') }}"
            class="asset-form">
            @csrf
            <div class="asset-form-header">Input Biaya Aktiva Gantung</div>
            <div class="asset-form-body">
                <div class="mb-3">
                    <label class="form-label d-block">Aset</label>
                    <div class="mode-grid">
                        <label class="mode-card {{ $modeAset === 'baru' ? 'is-active' : '' }}" data-mode-card="baru">
                            <input type="radio" name="mode_aset" value="baru" @checked($modeAset === 'baru')>
                            <span>
                                <strong class="d-block">Buat aset baru</strong>
                                <small class="text-muted">Contoh: Kandang Ayam A</small>
                            </span>
                        </label>
                        <label class="mode-card {{ $modeAset === 'lama' ? 'is-active' : '' }}" data-mode-card="lama">
                            <input type="radio" name="mode_aset" value="lama" @checked($modeAset === 'lama')>
                            <span>
                                <strong class="d-block">Tambah ke aset lama</strong>
                                <small class="text-muted">Untuk cicilan biaya berikutnya</small>
                            </span>
                        </label>
                    </div>
                </div>
                ...
                <div class="row g-3">
                    <div class="col-lg-6 mode-field mode-baru">
                        <label class="form-label" for="nama_aset">Nama Aset</label>
                        <input type="text" id="nama_aset" name="nama_aset" class="form-control"
                            value="{{ old('nama_aset') }}" placeholder="Contoh: Kandang Ayam A">
                    </div>
                    <div class="col-lg-6 mode-field mode-baru">
                        <label class="form-label" for="keterangan_aset">Catatan Aset</label>
                        <input type="text" id="keterangan_aset" name="keterangan_aset" class="form-control"
                            value="{{ old('keterangan_aset') }}" placeholder="Opsional">
                    </div>
                    <div class="col-lg-12 mode-field mode-lama">
                        ...
                        <select id="aktiva_gantung_id" name="aktiva_gantung_id" class="form-select select-search"
                            data-placeholder="Cari aset gantung">
                            <option value="">-- Pilih Aset --</option>
                            @foreach ($asetGantung as $aset)
                                <option value="{{ $aset->id }}" @selected(old('aktiva_gantung_id') == $aset->id)>
                ...
                <div class="d-flex justify-content-end gap-2 mt-3">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save me-1"></i> Simpan Aktiva Gantung
                    </button>
                </div>
            </div>
        </form>
    ...
    @section('scripts')
        <script>
            (function() {
                const modeRadios = document.querySelectorAll('input[name="mode_aset"]');
                ...
                function modeTerpilih() {
                    return document.querySelector('input[name="mode_aset"]:checked')?.value || 'baru';
                }

                function updateMode() {
                    const mode = modeTerpilih();

                    modeCards.forEach((card) => {
                        card.classList.toggle('is-active', card.dataset.modeCard === mode);
                    });

                    document.querySelectorAll('.mode-baru').forEach((field) => {
                        field.style.display = mode === 'baru' ? '' : 'none';
                    });
                    document.querySelectorAll('.mode-lama').forEach((field) => {
                        field.style.display = mode === 'lama' ? '' : 'none';
                    });

                    document.getElementById('nama_aset').required = mode === 'baru';
                    document.getElementById('aktiva_gantung_id').required = mode === 'lama';
                }
