<x-theme.app title="{{ $title }}" table="Y" sizeCard="10">
    <x-slot name="cardHeader">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div>
                ...
                <small class="text-muted">Edit pembayaran biaya listrik, PDAM, internet, atau biaya lainnya</small>
            </div>
            <a href="{{ route('pembukuan-baru.jurnal-umum.index', ['kelompok' => 'biaya']) }}"
                class="btn btn-outline-primary btn-sm">
                <i class="fas fa-arrow-left me-1"></i> Kembali ke Jurnal Biaya
            </a>
        </div>
    </x-slot>
    ...
    <x-slot name="cardBody">
        ...
        @php
            $oldDetail = old('detail', !empty($detail) ? $detail : [['id_akun_biaya' => '', 'keterangan' => '', 'jumlah' => '']]);
        @endphp

        <form method="POST" action="{{ route('pembukuan-baru.jurnal-umum.biaya.update', $nomor_transaksi) }}" class="cost-form">
            @csrf
            @method('PUT')
            <div class="cost-form-header">Edit Pembayaran Biaya ({{ $nomor_transaksi }})</div>
