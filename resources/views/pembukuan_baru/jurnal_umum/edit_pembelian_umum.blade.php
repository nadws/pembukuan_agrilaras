<x-theme.app title="{{ $title }}" table="Y" sizeCard="11">
    <x-slot name="cardHeader">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div>
                ...
                <small class="text-muted">Nomor Transaksi: {{ $nomorTransaksi }}</small>
            </div>
    ...
    <x-slot name="cardBody">
        ...
        @php
            ...
            if (!$oldDetail) {
                $oldDetail = [];
                foreach ($rowsDebit as $row) {
                    // Try to match product by name or keyword from description
                    $deskripsi = $row->deskripsi ?? '';
                    $matchedProduct = $produkList->first(function($p) use ($deskripsi) {
                        return str_contains(strtolower($deskripsi), strtolower($p->nm_produk));
                    });

                    $oldDetail[] = [
                        'id_produk' => $matchedProduct ? $matchedProduct->id_produk : ($produkList->first()->id_produk ?? ''),
                        'qty' => 1,
                        'satuan' => $matchedProduct ? ($matchedProduct->nm_satuan ?? 'PCS') : 'PCS',
                        'harga_satuan' => (float) $row->debit,
                        'id_akun_debit' => $row->id_akun_perkiraan,
        ...
        <form method="POST" action="{{ route('pembukuan-baru.jurnal-umum.pembelian-umum.update', $nomorTransaksi) }}" class="pu-form">
            @csrf
            @method('PUT')
            <div class="pu-form-header">Edit Transaksi Pembelian Umum</div>
            <div class="pu-form-body">
                <div class="row g-3">
                    ...
                    <div class="col-lg-3 col-md-6">
                        ...
                        <input type="text" id="nomor_transaksi" class="form-control bg-light"
                            value="{{ $nomorTransaksi }}" readonly>
                    </div>
                    <div class="col-lg-6">
                        ...
                        <select id="id_akun_pembayaran" name="id_akun_pembayaran" class="form-select select-search"
                            ...
                            @foreach ($akunPembayaran as $akun)
                                <option value="{{ $akun->id_akun_perkiraan }}" @selected(old('id_akun_pembayaran', $idAkunPembayaran) == $akun->id_akun_perkiraan)>
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" id="keterangan_global" name="keterangan_global" class="form-control"
                            value="{{ old('keterangan_global', $keteranganGlobal) }}" placeholder="Contoh: Pembelian perlengkapan dan peralatan kandang Martadah">
                    </div>
                </div>
                    ...
                    <div class="col-lg-6">
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-primary px-4">
                                <i class="fas fa-save me-1"></i> Update Pembelian Umum
                            </button>
                        </div>
                    </div>
                </div>
