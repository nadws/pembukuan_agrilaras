<x-theme.app title="{{ $title }}" sizeCard="12" cont="container-fluid">
    <x-slot name="cardHeader">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
                <h5 class="mb-0">Detail Penarikan Uang</h5>
    ...
    <x-slot name="cardBody">
        ...
        <div class="mb-3">
            ...
            <div class="text-muted">Periode {{ $start->translatedFormat('d F Y') }} s/d {{ $end->translatedFormat('d F Y') }} · {{ $selectedTransactionTypes ? collect($selectedTransactionTypes)->map(fn($type)=>$transactionTypeOptions[$type]['label'])->implode(', ') : 'Semua tipe transaksi' }}</div>
        </div>
        <form method="get" class="detail-filter mb-3">
            ...
            @foreach($selectedAccountIds as $accountId)<input type="hidden" name="akun_filter[]" value="{{ $accountId }}">@endforeach 
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-md-4"><div class="detail-kpi"><small>Debit − Kredit</small><strong>{{ $fmt($withdrawalTotal) }}</strong></div></div>
        </div>
