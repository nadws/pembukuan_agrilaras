<x-theme.app title="{{ $title }}" table="T" cont="container-fluid">
    <x-slot name="slot">
        ...
        <style>
            .widget-toolbar{display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-top:20px;padding:12px 16px;background:#fff;border:1px solid #dde5f2;border-radius:14px;box-shadow:0 9px 28px rgba(35,63,122,.07);font-size:13px;color:#536582}.widget-hint{margin-right:auto}.hidden-chips{display:flex;flex-wrap:wrap;gap:6px}.chip-tampil{border:1px dashed #8fa5d6;background:#f0f4ff;color:#2949ad;border-radius:20px;padding:5px 12px;font-size:12px;font-weight:700;cursor:pointer}.widget-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.widget-ctl{display:flex;gap:4px;flex:none}.widget-btn{width:30px;height:30px;display:inline-flex;align-items:center;justify-content:center;border:1px solid #dce5f3;background:#f5f8fd;color:#52648a;border-radius:8px;font-size:12px;cursor:pointer}.widget-btn:hover{background:#e8eeff;color:#2949ad}.widget-handle{cursor:grab;touch-action:none;user-select:none;-webkit-user-select:none}.widget-handle:active{cursor:grabbing}[data-widget].dragging{opacity:.45}[data-widget].drop-target{outline:2px dashed #435ebe;outline-offset:-2px}#widgetGrid{display:grid;grid-template-columns:repeat(12,1fr);gap:20px;align-items:start}.compare-panel{display:flex;flex-direction:column;height:560px}.feed-breakdown-body{flex:1 1 auto;min-height:0;max-height:none;overflow:auto}.widget-size{width:auto;min-width:66px;padding:0 10px;font-weight:800;font-size:11px}.stok-gudang{margin-bottom:16px}.stok-gudang:last-child{margin-bottom:0}.stok-total{display:flex;align-items:baseline;gap:8px;margin:2px 0 8px}.stok-total strong{font-size:26px;font-weight:900;color:#17366d;font-variant-numeric:tabular-nums}.stok-total span{font-size:12px;color:#75839a}.stok-rinci{border-top:1px dashed #e3e9f4}.stok-baris{display:flex;justify-content:space-between;gap:8px;font-size:12px;color:#536582;padding:6px 0;border-bottom:1px dashed #eef1f7}.stok-baris:last-child{border-bottom:0}.stok-baris span:last-child{white-space:nowrap;font-variant-numeric:tabular-nums;color:#193b78;font-weight:700}.stok-alert{background:#fdecec;border:1px solid #f5c2c2;color:#a12020;border-radius:12px;padding:10px 14px;font-size:12px;margin-bottom:14px}.stok-alert strong{font-weight:800}.stok-badge{display:inline-block;font-size:10px;font-weight:800;letter-spacing:.03em;border-radius:20px;padding:2px 10px;margin-left:6px;vertical-align:middle}.stok-bahaya{background:#fdecec;color:#c5303f;border:1px solid #f5c2c2}.stok-waspada{background:#fff7e0;color:#9a6b00;border:1px solid #f0d48a}.stok-aman{background:#e9f7ef;color:#2f8f57;border:1px solid #bfe3cd}.stok-nodata{background:#eef1f7;color:#75839a;border:1px solid #dde5f2}@media(max-width:900px){#widgetGrid{grid-template-columns:1fr}#widgetGrid>[data-widget]{grid-column:span 12 !important}.compare-panel{height:auto}.feed-breakdown-body{max-height:450px}}
        </style>
        <main class="compare-dashboard">
            ...
            <aside class="compare-panel feed-breakdown" data-widget="stok" data-span="{{ $widgetSpan['stok'] ?? 4 }}" style="order:{{ $widgetOrder['stok'] ?? 5 }};grid-column:span {{ $widgetSpan['stok'] ?? 4 }}" @if(in_array('stok', $widgetHidden)) hidden @endif>
                <div class="compare-panel-head"><h2 class="compare-panel-title">Stok Telur</h2><div class="compare-note">Posisi gudang saat ini</div></div>
                <div class="feed-breakdown-body">
                    @forelse($stokTelur as $idGudang => $rows)
                        @php($namaGudang = $rows->first()->gudang ?: 'Gudang '.$idGudang)
                        <div class="stok-gudang">
                            <h3 class="feed-house">{{ $namaGudang === 'Alpa' ? 'Banjarmasin (Alpa)' : $namaGudang }}</h3>
                            <div class="stok-total"><strong>{{ number_format($rows->sum('kg'), 1, ',', '.') }}</strong><span>kg &middot; {{ number_format($rows->sum('pcs'), 0, ',', '.') }} butir</span></div>
                            @if($rows->contains(fn ($i) => (float) $i->pcs != 0 || (float) $i->kg != 0))
                            <div class="stok-rinci">
                                @foreach($rows as $item)
                                    @if((float) $item->pcs == 0 && (float) $item->kg == 0)
                                        @continue
                                    @endif
                                    <div class="stok-baris"><span>{{ $item->produk ?: '-' }}</span><span>{{ number_format($item->pcs, 0, ',', '.') }} btr &middot; {{ number_format($item->kg, 1, ',', '.') }} kg</span></div>
                                @endforeach
                            </div>
                            @endif
                        </div>
                    @empty
                        <div class="compare-empty" style="height:200px">Belum ada stok telur.</div>
                    @endforelse
                </div>
            </aside>
            <aside class="compare-panel feed-breakdown" data-widget="stok-pakan" data-span="{{ $widgetSpan['stok-pakan'] ?? 4 }}" style="order:{{ $widgetOrder['stok-pakan'] ?? 6 }};grid-column:span {{ $widgetSpan['stok-pakan'] ?? 4 }}" @if(in_array('stok-pakan', $widgetHidden)) hidden @endif>
                <div class="compare-panel-head"><h2 class="compare-panel-title">Stok Pakan</h2><div class="compare-note">Hanya yang ada stok &middot; estimasi dari rata-rata pakai 7 hari terakhir</div></div>
                <div class="feed-breakdown-body">
                    @php($pakanBahaya = $stokPakan->where('status', 'bahaya')->values())
                    @if($pakanBahaya->isNotEmpty())
                        <div class="stok-alert"><i class="fas fa-exclamation-triangle me-1"></i>Stok menipis:
                            @foreach($pakanBahaya as $b)
                                <div><strong>{{ $b->nama }}</strong> tersisa &plusmn;{{ number_format($b->sisa_hari, 1, ',', '.') }} hari</div>
                            @endforeach
                        </div>
                    @endif
                    @forelse($stokPakan as $item)
                        <div class="stok-gudang">
                            <h3 class="feed-house">{{ $item->nama }}<span class="stok-badge stok-{{ $item->status }}">{{ $item->status === 'bahaya' ? '≤3 hari' : ($item->status === 'waspada' ? '≤7 hari' : ($item->status === 'aman' ? 'aman' : 'tanpa data')) }}</span></h3>
                            <div class="stok-total"><strong>{{ number_format($item->stok_gr, 0, ',', '.') }}</strong><span>gr &middot; {{ number_format($item->stok_sak, 2, ',', '.') }} sak</span></div>
                            <div class="stok-rinci">
                                <div class="stok-baris"><span>Rata-rata pakai</span><span>{{ number_format($item->rata_gr / 1000, 1, ',', '.') }} kg/hari</span></div>
                                <div class="stok-baris"><span>Estimasi tahan</span><span>{{ $item->sisa_hari === null ? 'tanpa data pakai' : '±'.number_format($item->sisa_hari, 1, ',', '.').' hari' }}</span></div>
                            </div>
                        </div>
                    @empty
                        <div class="compare-empty" style="height:200px">Semua stok pakan kosong.</div>
                    @endforelse
                </div>
            </aside>
        </div>
        ...
        <script>
            const WIDGET_JUDUL={pakan:'Pemakaian Pakan','pakan-rincian':'Pakan per Kandang',telur:'Produksi Telur per Kandang','laba-rugi':'Laba Rugi per Kandang',piutang:'Piutang Telur Belum Lunas',stok:'Stok Telur','stok-pakan':'Stok Pakan'};
            const WIDGET_DEFAULT=['pakan','pakan-rincian','telur','laba-rugi','piutang','stok','stok-pakan'];
            const BOLEH_UBAH=@json($bolehUbah);
            const SPAN_DEFAULT={pakan:8,'pakan-rincian':4,telur:8,'laba-rugi':4,piutang:12,stok:12,'stok-pakan':4};
            ...
            function initWidgetGrid(){
                ...
                if(!BOLEH_UBAH){emptyNote.hidden=cards().some(c=>!c.hidden);return;}
