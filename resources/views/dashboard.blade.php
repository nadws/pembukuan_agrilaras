<x-theme.app title="{{ $title }}" table="T" cont="container-fluid">
    <x-slot name="slot">
        ...
        <style>
            .widget-toolbar{display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-top:20px;padding:12px 16px;background:#fff;border:1px solid #dde5f2;border-radius:14px;box-shadow:0 9px 28px rgba(35,63,122,.07);font-size:13px;color:#536582}.widget-hint{margin-right:auto}.hidden-chips{display:flex;flex-wrap:wrap;gap:6px}.chip-tampil{border:1px dashed #8fa5d6;background:#f0f4ff;color:#2949ad;border-radius:20px;padding:5px 12px;font-size:12px;font-weight:700;cursor:pointer}.widget-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.widget-ctl{display:flex;gap:4px;flex:none}.widget-btn{width:30px;height:30px;display:inline-flex;align-items:center;justify-content:center;border:1px solid #dce5f3;background:#f5f8fd;color:#52648a;border-radius:8px;font-size:12px;cursor:pointer}.widget-btn:hover{background:#e8eeff;color:#2949ad}.widget-handle{cursor:grab;touch-action:none;user-select:none;-webkit-user-select:none}.widget-handle:active{cursor:grabbing}[data-widget].dragging{opacity:.45}[data-widget].drop-target{outline:2px dashed #435ebe;outline-offset:-2px}#widgetGrid{display:grid;grid-template-columns:repeat(12,1fr);gap:20px;align-items:start}.compare-panel{display:flex;flex-direction:column;height:560px}.feed-breakdown-body{flex:1 1 auto;min-height:0;max-height:none;overflow:auto}.widget-size{width:auto;min-width:66px;padding:0 10px;font-weight:800;font-size:11px}.stok-gudang{margin-bottom:16px}.stok-gudang:last-child{margin-bottom:0}.stok-total{display:flex;align-items:baseline;gap:8px;margin:2px 0 8px}.stok-total strong{font-size:26px;font-weight:900;color:#17366d;font-variant-numeric:tabular-nums}.stok-total span{font-size:12px;color:#75839a}.stok-rinci{border-top:1px dashed #e3e9f4}.stok-baris{display:flex;justify-content:space-between;gap:8px;font-size:12px;color:#536582;padding:6px 0;border-bottom:1px dashed #eef1f7}.stok-baris:last-child{border-bottom:0}.stok-baris span:last-child{white-space:nowrap;font-variant-numeric:tabular-nums;color:#193b78;font-weight:700}@media(max-width:900px){#widgetGrid{grid-template-columns:1fr}#widgetGrid>[data-widget]{grid-column:span 12 !important}.compare-panel{height:auto}.feed-breakdown-body{max-height:450px}}
        </style>
        ...
        <script>
            const WIDGET_JUDUL={pakan:'Pemakaian Pakan','pakan-rincian':'Pakan per Kandang',telur:'Produksi Telur per Kandang','laba-rugi':'Laba Rugi per Kandang',piutang:'Piutang Telur Belum Lunas',stok:'Stok Telur'};
            const WIDGET_DEFAULT=['pakan','pakan-rincian','telur','laba-rugi','piutang','stok'];
            const BOLEH_UBAH=@json($bolehUbah);
            const SPAN_DEFAULT={pakan:8,'pakan-rincian':4,telur:8,'laba-rugi':4,piutang:12,stok:12};
